...
app:
	...
	mov w11, 0x001F    //0x001F = azul  (para uso tardio)
...
figures:
	//dibujo de paredes: contornos del laberinto
	//x4, inicio x, x5: inicio y, x6: final y: x7, final x
	mov x4,220		//inicio de la figura, argumento inicio x
	mov x5,0		//argumento inicio y
	mov x6,10		//argumento final y
	mov x7,512		// argumento final x
	mov w8,0x001E      // argumento de color de rectangulo, azul2
	bl rectangle		 // llama a la funcion que dibuja la figura	
		//comentarios son redundantes así que se abrevian para el mismo tipo	
	mov x4,502
	mov x5,0
	mov x6,512
	mov x7,512
	...
	//dibujo de paredes: paredes internas
	//primera fila
	mov x4,146
	mov x5,0
	mov x6,73
	mov x7,156
	mov w8,0xF000       // argumento de color de rectangulo, rojo
	bl rectangle
	...
	mov x5,438
	mov x6,448
	mov x7,156  //caso especial porque se corta 
	bl rectangle
...
player1:
	...
	add x12,x8,25            //final del eje x del jugador (distancia impar para resultados óptimos) basicamente determina el TAMANIO
	add x25,x9,13		//final eje y del jugador (deveria ser x12 dividido 2 pero no hay divisiones)
player:
	bl inputRead
	...
	and w23,w22,#0x4000  //filtrar solamente este valor de w22 y almacenarlo en w23 (descartando posibles bits activados)
	...
	b.eq movizq
	b playerdraw      //Si ninguno esta presionado no hay movimiento, a esperar
...
//evento
	...
	b.lt unevent		// apaga el led verde si esta en el eje x inapropiado
	cmp x8,292		//verifica si esta en la porcion correcta eje x del laberinto
	b.gt unevent		// apaga el led verde si esta en el eje x inapropiado
	cmp x25,438		//verifica si esta en la porcion correcta del eje y del laberinto
	b.lt unevent		// apaga el led verde si esta en el eje y inapropiado
	mov w22, 0x4
